...
Capability roles are separate from model names. `config/roles.json` is authoritative; regenerate profiles after editing it. Defaults are FRONTIER = Astra / Low, SENIOR = Sol / Medium, WORKHORSE = Terra / Medium, MECHANICAL = Luna / Low. The root Sol is the thin coordinator, not an automatically created child host. A bounded SENIOR subagent may reason over supplied evidence, but it remains tool-thin under the same session guard and does not perform repository labor.
...
2. Mission authority and the final criterion decision remain with the thin senior host. Repository intake, reconstruction, decomposition and final evidence-matrix production go to a worker. Judgment/audit without reconnaissance or compressed evidence returns to a worker to collect facts.
...
4. Deep or ambiguous repository work uses WORKHORSE at higher effort; a SENIOR pass may reason over its bounded evidence but does not execute repository tools. Ordinary implementation/recon/repair uses workhorse. Known, verifiable, non-consequential mechanical work uses mechanical. Task volume does not raise the role.
